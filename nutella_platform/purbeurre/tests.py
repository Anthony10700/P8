...
class CreateUserTests(TestCase):
    ...
    def test_accound(self):
        ...
        response = self.client.get('/account.html')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['account_info']["Email"], "Test-accound@monmail.com")


    def test_history(self):
        response = self.client.get('/history.html')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['title'], "Vous n'êtes pas connecté.")

        self.make_account()
        self.client.login(username='Test_accound', password='Test_psw')
        response = self.client.get('/history.html')
        self.assertEqual(response.context['title'], "Historique de vos articles")
    ...
    def make_account(self):
        info = {"inputUsername":"Test_accound","inputemail":"Test-accound@monmail.com","inputPassword1":"Test_psw","inputPassword2":"Test_psw","inputNom":"Test_Nom","inputprenom":"Test_prenom"}
        response = self.client.post('/sign_in.html',data=info)
        self.assertEqual(response.status_code, 302)    

        response = self.client.get('/account.html')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context['account_info']["Email"], info["inputemail"])
